...
// 2. ดึงข้อมูลเฉพาะของ Honkai: Star Rail
$sql = "SELECT * FROM characters WHERE game = 'Honkai: Star Rail' AND category = ? AND status = 'approved' ORDER BY id DESC";
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    if ($result && $result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            // ลอจิกการแสดงป้ายกำกับ (ปรับให้เข้ากับระบบของ Star Rail)
            $meta_label = "";
        ...
        echo '  <div style="font-size: 40px; margin-bottom: 15px; color: #f2c94c;">✧</div>';
...
$display_title = ($current_cat === 'item') ? "คลังแสงและ Light Cones (Items)" : (($current_cat === 'affiliation') ? "ฝ่ายและองค์กร (Factions)" : "รายชื่อตัวละคร (Characters)");
?>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Noto Sans Thai', sans-serif; }
        ...
        /* Hero Banner (ใช้รูปปกของ Star Rail ที่เคยอัปโหลดไว้) */
        .hero { 
            height: 350px; 
            background: linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.95)), url('Honkai star rail/Home Honkai star rail.jpg'); 
        ...
        .hero h1 { font-size: 48px; font-weight: 700; text-shadow: 0 0 30px rgba(242, 201, 76, 0.8); margin-bottom: 10px; letter-spacing: 2px; }
        ...
        .side-item.active { border-color: var(--primary); background: rgba(242, 201, 76, 0.15); box-shadow: 0 0 20px rgba(242, 201, 76, 0.2); transform: scale(1.05); }
        ...
        .char-square-btn:hover { transform: translateY(-12px); border-color: var(--primary); box-shadow: 0 15px 35px rgba(242, 201, 76, 0.35); }
        ...
        .element-overlay { position: absolute; top: 15px; left: 15px; background: rgba(0, 0, 0, 0.8); backdrop-filter: blur(8px); color: var(--primary); padding: 6px 12px; border-radius: 10px; font-size: 11px; font-weight: 700; z-index: 2; border: 1px solid rgba(242, 201, 76, 0.4); text-transform: uppercase; }
        ...
        .floating-btn { position: fixed; bottom: 40px; right: 40px; width: 70px; height: 70px; background: linear-gradient(135deg, var(--primary), #d4af37); border-radius: 50%; display: flex; justify-content: center; align-items: center; box-shadow: 0 10px 30px rgba(242, 201, 76, 0.4); z-index: 1000; transition: 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); border: 2px solid #fff; text-decoration: none; }
...
<body>
    <video autoplay muted loop playsinline id="bg-video">
        <source src="Video/Honkai%20star%20rail.mp4" type="video/mp4">
    </video>

    <nav>
        <div style="font-size: 20px; font-weight: 700; color: #f2c94c; letter-spacing: 2px;">HOYOVERSE <span style="color:#fff; font-weight:300;">ARCHIVE</span></div>
        <a href="index.php" class="btn-home">กลับหน้าหลัก</a>
    </nav>

    <div class="hero">
        <h1>Honkai: Star Rail Archive</h1>
        <p class="slogan">"May this journey lead us starward"</p>
    </div>

    <div class="archive-frame">
        <div class="archive-sidebar">
            <div class="side-item <?php echo ($current_cat === 'character') ? 'active' : ''; ?>" onclick="switchCat('character', this)">
                <svg width="30" height="30" viewBox="0 0 24 24" fill="<?php echo ($current_cat === 'character') ? '#f2c94c' : '#666'; ?>"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
                <span>Characters</span>
            </div>
            <div class="side-item <?php echo ($current_cat === 'item') ? 'active' : ''; ?>" onclick="switchCat('item', this)">
                <svg width="25" height="25" viewBox="0 0 24 24" fill="<?php echo ($current_cat === 'item') ? '#f2c94c' : '#666'; ?>"><path d="M21 16.5C21 16.88 20.79 17.21 20.47 17.38L12.57 21.82C12.41 21.94 12.21 22 12 22C11.79 22 11.59 21.94 11.43 21.82L3.53 17.38C3.21 17.21 3 16.88 3 16.5V7.5C3 7.12 3.21 6.79 3.53 6.62L11.43 2.18C11.59 2.06 11.79 2 12 2C12.21 2 12.41 2.06 12.57 2.18L20.47 6.62C20.79 6.79 21 7.12 21 7.5V16.5Z"/></svg>
                <span>Light Cones</span>
            </div>
            <div class="side-item <?php echo ($current_cat === 'affiliation') ? 'active' : ''; ?>" onclick="switchCat('affiliation', this)">
                <svg width="25" height="25" viewBox="0 0 24 24" fill="<?php echo ($current_cat === 'affiliation') ? '#f2c94c' : '#666'; ?>"><path d="M12 2L4.5 20.29L5.21 21L12 18L18.79 21L19.5 20.29L12 2Z"/></svg>
                <span>Factions</span>
            </div>
        </div>
    ...
    <a href="submit_info.php" class="floating-btn" title="เพิ่มข้อมูลใหม่">
        <svg viewBox="0 0 24 24" width="32" height="32" fill="#000"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
    </a>
    ...
    function switchCat(cat, element) {
        ...
        // ชี้ไปที่ไฟล์ starrail.php
        fetch(`starrail.php?cat=${cat}&ajax=1`)
            .then(response => response.text())
            .then(data => {
                container.innerHTML = data;
                container.classList.remove('loading-overlay');
                
                // อัปเดตหัวข้อให้ตรงกับ Star Rail
                const titles = { 
                    'character': 'รายชื่อตัวละคร (Characters)', 
                    'item': 'คลังแสงและ Light Cones (Items)', 
                    'affiliation': 'ฝ่ายและองค์กร (Factions)' 
                };
                title.innerText = titles[cat];
                
                // จัดการสถานะ Sidebar สีทอง (Primary)
                document.querySelectorAll('.side-item').forEach(el => {
                    el.classList.remove('active');
                    el.querySelector('svg').setAttribute('fill', '#666');
                });
                element.classList.add('active');
                element.querySelector('svg').setAttribute('fill', '#f2c94c'); 
                
                window.history.pushState({cat: cat}, '', `starrail.php?cat=${cat}`);
            })
            .catch(err => { 
                console.error(err); 
                container.classList.remove('loading-overlay'); 
