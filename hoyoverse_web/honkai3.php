...
// 2. ดึงข้อมูลเฉพาะของ Honkai Impact 3rd
$sql = "SELECT * FROM characters WHERE game = 'Honkai Impact 3rd' AND category = ? AND status = 'approved' ORDER BY id DESC";
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    if ($result && $result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            // ลอจิกการแสดงป้ายกำกับ (ปรับให้เข้ากับระบบของ Honkai Impact 3rd)
            $meta_label = "";
            ...
            echo '  <div class="char-img-box"><img src="Image/' . htmlspecialchars($safe_icon) . '" loading="lazy"></div>';
        ...
        echo '  <div style="font-size: 40px; margin-bottom: 15px; color: #ff3399;">✧</div>';
...
$display_title = ($current_cat === 'item') ? "คลังอาวุธและสติกมาตา (Equipment)" : (($current_cat === 'affiliation') ? "องค์กรและสังกัด (Factions)" : "รายชื่อวาลคีเรีย (Valkyries)");
?>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Noto Sans Thai', sans-serif; }
        ...
        .btn-home:hover { background: var(--primary); color: #fff; box-shadow: 0 0 25px var(--primary); transform: scale(1.05); }

        /* Hero Banner (ใช้รูปปกของ Honkai Impact 3) */
        .hero { 
            height: 350px; 
            background: linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.95)), url('Honaki Impact 3/Home Honkai impact 3.png'); 
        ...
        .hero h1 { font-size: 48px; font-weight: 700; text-shadow: 0 0 30px rgba(255, 51, 153, 0.8); margin-bottom: 10px; letter-spacing: 2px; }
        ...
        .side-item.active { border-color: var(--primary); background: rgba(255, 51, 153, 0.15); box-shadow: 0 0 20px rgba(255, 51, 153, 0.2); transform: scale(1.05); }
        ...
        .char-square-btn:hover { transform: translateY(-12px); border-color: var(--primary); box-shadow: 0 15px 35px rgba(255, 51, 153, 0.35); }
        ...
        .element-overlay { position: absolute; top: 15px; left: 15px; background: rgba(0, 0, 0, 0.8); backdrop-filter: blur(8px); color: var(--primary); padding: 6px 12px; border-radius: 10px; font-size: 11px; font-weight: 700; z-index: 2; border: 1px solid rgba(255, 51, 153, 0.4); text-transform: uppercase; }
        ...
        .floating-btn { position: fixed; bottom: 40px; right: 40px; width: 70px; height: 70px; background: linear-gradient(135deg, var(--primary), var(--secondary)); border-radius: 50%; display: flex; justify-content: center; align-items: center; box-shadow: 0 10px 30px rgba(255, 51, 153, 0.4); z-index: 1000; transition: 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); border: 2px solid #fff; text-decoration: none; }
...
<body>
    ...
    <nav>
        <div style="font-size: 20px; font-weight: 700; color: #ff3399; letter-spacing: 2px;">HOYOVERSE <span style="color:#fff; font-weight:300;">ARCHIVE</span></div>
        <a href="index.php" class="btn-home">กลับหน้าหลัก</a>
    </nav>

    <div class="hero">
        <h1>Honkai Impact 3rd Archive</h1>
        <p class="slogan">"Fight for all that is beautiful in the world"</p>
    </div>

    <div class="archive-frame">
        <div class="archive-sidebar">
            <div class="side-item <?php echo ($current_cat === 'character') ? 'active' : ''; ?>" onclick="switchCat('character', this)">
                <svg width="30" height="30" viewBox="0 0 24 24" fill="<?php echo ($current_cat === 'character') ? '#ff3399' : '#666'; ?>"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
                <span>Valkyries</span>
            </div>
            <div class="side-item <?php echo ($current_cat === 'item') ? 'active' : ''; ?>" onclick="switchCat('item', this)">
                <svg width="25" height="25" viewBox="0 0 24 24" fill="<?php echo ($current_cat === 'item') ? '#ff3399' : '#666'; ?>"><path d="M21 16.5C21 16.88 20.79 17.21 20.47 17.38L12.57 21.82C12.41 21.94 12.21 22 12 22C11.79 22 11.59 21.94 11.43 21.82L3.53 17.38C3.21 17.21 3 16.88 3 16.5V7.5C3 7.12 3.21 6.79 3.53 6.62L11.43 2.18C11.59 2.06 11.79 2 12 2C12.21 2 12.41 2.06 12.57 2.18L20.47 6.62C20.79 6.79 21 7.12 21 7.5V16.5Z"/></svg>
                <span>Equipment</span>
            </div>
            <div class="side-item <?php echo ($current_cat === 'affiliation') ? 'active' : ''; ?>" onclick="switchCat('affiliation', this)">
                <svg width="25" height="25" viewBox="0 0 24 24" fill="<?php echo ($current_cat === 'affiliation') ? '#ff3399' : '#666'; ?>"><path d="M12 2L4.5 20.29L5.21 21L12 18L18.79 21L19.5 20.29L12 2Z"/></svg>
                <span>Factions</span>
            </div>
        </div>
    ...
    function switchCat(cat, element) {
        ...
        // ชี้ไปที่ไฟล์ honkai3.php
        fetch(`honkai3.php?cat=${cat}&ajax=1`)
            .then(response => response.text())
            .then(data => {
                container.innerHTML = data;
                container.classList.remove('loading-overlay');
                
                // อัปเดตหัวข้อให้ตรงกับ Honkai Impact 3rd
                const titles = { 
                    'character': 'รายชื่อวาลคีเรีย (Valkyries)', 
                    'item': 'คลังอาวุธและสติกมาตา (Equipment)', 
                    'affiliation': 'องค์กรและสังกัด (Factions)' 
                };
                title.innerText = titles[cat];
                
                // จัดการสถานะ Sidebar สีชมพู (Primary)
                document.querySelectorAll('.side-item').forEach(el => {
                    el.classList.remove('active');
                    el.querySelector('svg').setAttribute('fill', '#666');
                });
                element.classList.add('active');
                element.querySelector('svg').setAttribute('fill', '#ff3399'); 
                
                window.history.pushState({cat: cat}, '', `honkai3.php?cat=${cat}`);
            })
            .catch(err => { 
                console.error(err); 
                container.classList.remove('loading-overlay'); 
